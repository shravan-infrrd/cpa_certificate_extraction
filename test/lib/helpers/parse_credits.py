    ...
    def parse_between_lines(self):
        ...
        for index, content in enumerate(self.contents):
            for kw in post_keywords:
                if kw in content:
                    print("6.15*************PARSING*CREDITS*****************", kw)
                    values = remove_extra_spaces( self.contents[ index + 1 ].strip() )
                    for val in values:

                        if hasNumbers(val):
                            print("6.1*************PARSING*CREDITS*****************", val)
                            self.credits = val
                            self.credits = self.get_credits()
    ...
    def build_field_of_study(self):
        if len(self.fos) == 1:
            ...
            try:
                ...
                self.field_of_study.append({"name": self.fos[0], "credits": "", "score": ""})
            ...
            for fos in self.fos:
                ...
                if credits != "":
                    try:
                        ...
                        print("1**CREDITS_INISIDE**", credit)
                        if credit < float(20):
